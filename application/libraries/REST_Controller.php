...
abstract class REST_Controller extends \CI_Controller {
    ...
    public function __construct($config = 'rest')
    {
            ...
            ($this->config->item('allow_auth_and_keys') === TRUE && $this->_allow === TRUE)))
        {
			...
			if ($rest_auth == 'basic'){
				$this->validation_rest_auth_basic();
			}
	...
	function validation_rest_auth_basic(){
		if ( ! isset($this->config->item('rest_valid_logins')[$this->input->server('PHP_AUTH_USER')])){
			echo json_encode(array('status'=> false , 'error'=>'Invalid User or Password'));
			//$this->response(array('status' => false, 'error' => 'Not authorized'), 401);
			exit ;
		} elseif ($this->config->item('rest_valid_logins')[$this->input->server('PHP_AUTH_USER')] != $this->input->server('PHP_AUTH_PW') ){
			//$this->response(array('status' => false, 'error' => 'Not authorized'), 401);
			echo json_encode(array('status'=> false , 'error'=>'Invalid User or Password'));
			exit;
		} 
